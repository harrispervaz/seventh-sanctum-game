...
def create_starter_deck(faction):
    ...
    # Get all cards for the faction
    faction_cards = [c for c in CARD_DATABASE['cards'] if c['faction'] == faction]
    generic_cards = [c for c in CARD_DATABASE['cards'] if c['faction'] == 'Generic']
    
    # Add duplicates to reach 42 cards
    available = faction_cards + generic_cards
    
    # Simple approach: add each card, then fill with random duplicates
    for card in faction_cards:
        deck.append(card['id'])
    
    for card in generic_cards[:6]:  # Add some generic cards
        deck.append(card['id'])
    
    # Fill to 42 with random duplicates
    while len(deck) < 42:
        random_card = random.choice(available)
        deck.append(random_card['id'])
