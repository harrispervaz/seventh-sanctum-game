...
async function playAttackLine(attackerElement, defenderElement) {
    ...
    let lineWidth = 0;
    const lineDrawSpeed = distance / 200; // pixels per ms
    ...
    let pulsePos = 0;
    const pulseSpeed = distance / 300; // pixels per ms
    const pulseInterval = setInterval(() => {
        pulsePos += pulseSpeed * 16;
        if (pulsePos >= distance) {
            clearInterval(pulseInterval);
            // Remove line and pulse after impact
            setTimeout(() => line.remove(), 100);
        }
        pulse.style.left = pulsePos + 'px';
    }, 16);
    
    // Wait for pulse to reach target
...
async function playCombatAnimation(attackerElement, defenderElement, damage) {
    ...
    // 2. Attack line with energy pulse
