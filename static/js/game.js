...
function renderField(player, field) {
    ...
    // Check if field was newly played or replaced (for opponent animation)
    const wasNewlyPlayed = !hadCard && field;
    const wasReplaced = hadCard && field && oldFieldId !== field.id;
    ...
    if (field) {
        ...
        // Play slam animation for OPPONENT when field is newly played or replaced
        if (player === 'opponent' && (wasNewlyPlayed || wasReplaced)) {
            playFieldAnimation(field.id, wasReplaced, 'opponent');
        }
    } else {
        slot.innerHTML = '<small>FIELD</small>';
