...
async function advancePhase() {
    ...
    try {
        ...
        if (combatMessages && Array.isArray(combatMessages) && combatMessages.length > 0) {
            console.log('⚔️ ========================================');
            console.log('⚔️ COMBAT MESSAGES FOUND!');
            console.log('⚔️ Source:', data.combat_log ? 'combat_log' : 'log');
            ...
            combatMessages.forEach(async (logItem, index) => {
                // Extract message text from object or use string directly
                let logMsg = logItem;
                if (typeof logItem === 'object' && logItem !== null) {
                    ...
                    console.log(`📦 [${index}] Object with message: "${logMsg}"`);
                } else {
                    console.log(`⚔️ [${index}] "${logMsg}"`);
                }
                
                if (typeof logMsg !== 'string') {
                    console.log(`⚠️ Message ${index} could not be converted to string`);
                    return;
                }
                
                console.log(`🔍 Parsing message ${index}: "${logMsg}"`);
                
                // Try multiple patterns to match combat
                const patterns = [
                    { name: 'Pattern 1', regex: /\(slot (\d+)\) attacks .* \(slot (\d+)\)/ },
                    { name: 'Pattern 2', regex: /slot (\d+) attacks slot (\d+)/ },
                    { name: 'Pattern 3', regex: /attacks.*from slot (\d+).*to slot (\d+)/ },
                ];
                ...
                for (const pattern of patterns) {
                    const match = logMsg.match(pattern.regex);
                    if (match) {
                        attackMatch = match;
                        matchedPattern = pattern.name;
                        console.log(`   ✓ ${pattern.name} matched:`, match);
                        break;
                    }
                }
                ...
                if (attackMatch && damageMatch) {
                    ...
                    console.log(`      Attacker zone: ${attackerZone}`);
                    console.log(`      Defender zone: ${defenderZone}`);
                    ...
                    if (attackerEl && defenderEl && damage > 0) {
                        ...
                        console.log('      ❌ CANNOT PLAY ANIMATION:');
                        console.log('         Has attacker?', !!attackerEl);
                        console.log('         Has defender?', !!defenderEl);
                        console.log('         Damage > 0?', damage > 0);
                    }
                } else {
                    console.log('   ⚠️ Could not parse attack/damage from message');
                    console.log('   Message was:', logMsg);
                }
            ...
            console.log('📦 No combat messages found in response');
        }
