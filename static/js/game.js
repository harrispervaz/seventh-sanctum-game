...
async function playTrapFlipAnimation(player, trapSlot, trap) {
    ...
    // STEP 1: Flip animation (0.6s)
    trapCardEl.classList.add('trap-flipping');
    
    // Wait for flip to complete
    await new Promise(resolve => setTimeout(resolve, 600));
    
    // STEP 2: Show the trap face (replace card back with actual trap card)
    // Remove the flip animation class
    trapCardEl.classList.remove('trap-flipping');
    
    // Replace card content with trap face
    trapCardEl.innerHTML = `
        <div class="card-header">
            <span class="card-name">${trap.name}</span>
            <span class="card-cost">${trap.cost} ⚡</span>
        </div>
        <div class="card-body">
            <div class="card-effect">${trap.effect}</div>
        </div>
        <div class="card-footer">
            <span class="card-type">TRAP</span>
            <span class="card-faction">${trap.faction}</span>
        </div>
    `;
    
    // Add revealed state with black glow
    trapCardEl.classList.add('trap-revealed');
    
    // STEP 3: Pause so player can read the trap (1.2s)
    await new Promise(resolve => setTimeout(resolve, 1200));
    
    // STEP 4: Discard animation (0.5s)
    trapCardEl.classList.remove('trap-revealed');
