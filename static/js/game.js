...
function updateUI() {
    ...
    // Update sci-fi UI displays (Phase 1)
    updateSciFiUI();
...
/* ═══════════════════════════════════════════════════════════════
   SCI-FI UI SYSTEM - Phase 1: Stats & Info Displays
   ═══════════════════════════════════════════════════════════════ */
...
function updateEnergyOrbs(player, currentEnergy, maxEnergy = 5) {
    const container = document.querySelector(`#${player}-energy-orbs`);
    if (!container) return;
    
    // Clear existing orbs
    container.innerHTML = '';
    
    // Check if at max energy for special effect
    const isMaxEnergy = currentEnergy === maxEnergy;
    if (isMaxEnergy) {
        container.classList.add('max-energy');
    } else {
        container.classList.remove('max-energy');
    }
...
// Animate energy gain (orbs light up)
function animateEnergyGain(player, amount) {
    const container = document.querySelector(`#${player}-energy-orbs`);
    if (!container) return;
    
    const orbs = container.querySelectorAll('.energy-orb');
    let delay = 0;
    
    for (let i = 0; i < amount && i < orbs.length; i++) {
        setTimeout(() => {
            orbs[i].classList.remove('empty', 'used');
            orbs[i].classList.add('available', 'gaining');
            
            // Remove gaining class after animation
            setTimeout(() => {
                orbs[i].classList.remove('gaining');
            }, 600);
        }, delay);
...
// Animate energy spend (orbs deplete)
function animateEnergySpend(player, amount, currentEnergy) {
    const container = document.querySelector(`#${player}-energy-orbs`);
    if (!container) return;
    ...
    // Mark spent orbs
    for (let i = currentEnergy; i < currentEnergy + amount && i < orbs.length; i++) {
        orbs[i].classList.remove('available');
        orbs[i].classList.add('used');
    }
...
// ┌──────────────────────────────────────────────────────────────┐
// │ REACTOR GAUGE (Control Loss Display)                        │
// └──────────────────────────────────────────────────────────────┘

function updateReactorGauge(player, controlLoss, maxLoss = 3) {
    const fill = document.querySelector(`#${player}-reactor-fill`);
    const counter = document.querySelector(`#${player}-reactor-counter`);
    
    if (!fill || !counter) return;
    
    // Calculate fill percentage (0-100%)
    const fillPercent = (controlLoss / maxLoss) * 100;
    fill.style.height = fillPercent + '%';
    
    // Update counter text
    counter.textContent = controlLoss + '/' + maxLoss;
    
    // Update danger state
    fill.classList.remove('danger-low', 'danger-medium', 'danger-high');
    
    if (controlLoss === 0) {
        fill.classList.add('danger-low');
    } else if (controlLoss === 1) {
        fill.classList.add('danger-medium');
    } else if (controlLoss >= 2) {
        fill.classList.add('danger-high');
    }
}

// Animate control loss gain (reactor fills, screen shakes)
function animateControlLossGain(player, newLoss) {
    updateReactorGauge(player, newLoss);
    ...
    showActionNotification('CONTROL LOSS!', player, 'control-loss');
}
...
function updatePhaseIndicator(phase) {
    ...
    const text = indicator.querySelector('.phase-text');
    ...
    switch(phase) {
        case 'deploy':
        case 'start': // Start phase shows as deploy
...
function updateTurnCounter(turnNumber) {
    // Update both old and new turn displays
    const oldCounter = document.getElementById('turn-number');
    const newCounter = document.getElementById('turn-number-display');
    
    if (oldCounter) {
        oldCounter.textContent = turnNumber;
    }
    if (newCounter) {
        newCounter.textContent = turnNumber;
    }
}
...
function updateActivePlayer(activePlayer) {
    const yourPanel = document.getElementById('stat-panel-player');
    const opponentPanel = document.getElementById('stat-panel-opponent');
    
    if (!yourPanel || !opponentPanel) return;
    
    // Remove active states
    yourPanel.classList.remove('active-turn', 'ai-thinking');
    opponentPanel.classList.remove('active-turn', 'ai-thinking');
    ...
    if (activePlayer === 0) {
        ...
        if (isAITurn) {
            opponentPanel.classList.add('ai-thinking');
        }
    }
}
...
function updateDeckCounter(player, deckSize) {
    const counter = document.getElementById(`${player}-deck-count`);
    if (counter) {
        counter.textContent = deckSize;
    }
}

function updateHandCounter(player, handSize) {
    const counter = document.getElementById(`${player}-hand-count`);
    if (counter) {
...
// ┌──────────────────────────────────────────────────────────────┐
// │ ACTION NOTIFICATIONS (Floating Text)                         │
// └──────────────────────────────────────────────────────────────┘
...
function showActionNotification(message, player, type) {
    ...
    const panel = document.getElementById(`stat-panel-${player === 'your' ? 'player' : 'opponent'}`);
    if (panel) {
...
function updateSciFiUI() {
    if (!gameState) return;
    
    // Update energy displays
    updateEnergyOrbs('your', gameState.you.energy);
    updateEnergyOrbs('opponent', gameState.opponent.energy);
    
    // Update reactor gauges
    updateReactorGauge('your', gameState.you.control_loss);
    updateReactorGauge('opponent', gameState.opponent.control_loss);
    
    // Update phase indicator
    updatePhaseIndicator(gameState.phase);
    
    // Update turn counter
    updateTurnCounter(gameState.turn);
    
    // Update active player indicator
    updateActivePlayer(gameState.active_player);
    
    // Update deck counters
    updateDeckCounter('your', gameState.you.deck_count);
    updateDeckCounter('opponent', gameState.opponent.deck_count);
    
    // Update hand counters
    updateHandCounter('your', gameState.you.hand.length);
    updateHandCounter('opponent', gameState.opponent.hand_count);
}
