...
// AI pacing system - makes AI feel more human by staggering actions
let aiActionCounter = 0;  // Counts actions this AI turn
let aiActionDelay = 650;  // Base delay between AI actions (ms)
let isAITurn = false;     // Track if we're currently in AI turn
let aiTurnIteration = 0;  // Track which iteration of AI turn we're on

// Reset AI action counter
function resetAIActionCounter() {
    aiActionCounter = 0;
    aiTurnIteration = 0;
    console.log('🤖 AI action counter reset');
}

// Get delay for next AI action (progressive delay)
function getNextAIActionDelay() {
    if (!isAITurn) return 0;  // No delay during player turn
    
    const delay = aiActionCounter * aiActionDelay;
    aiActionCounter++;
    console.log(`⏱️ AI action ${aiActionCounter}: ${delay}ms delay`);
    return delay;
}
...
function renderBattlefield(player, units) {
    for (let i = 0; i < 5; i++) {
        ...
        // If this is a NEW AI card on FIRST iteration (when deploys happen), delay its appearance
        const isOpponent = player === 'opponent';
        const isNewCard = !hadCard && units[i];
        const isFirstIteration = aiTurnIteration === 1;
        
        if (isNewCard && isOpponent && isAITurn && isFirstIteration) {
            // Don't render yet - schedule it for later
            const delay = getNextAIActionDelay();
            const unitData = units[i]; // Capture unit data NOW
            const slotIndex = i; // Capture index NOW
            
            console.log(`⏰ Scheduling card for slot ${slotIndex} with ${delay}ms delay`);
            
            setTimeout(() => {
                console.log(`✨ Rendering delayed card for slot ${slotIndex}`);
                // Find the slot again (in case DOM was updated)
                const targetSlot = document.querySelector(`[data-zone="${player}-unit"][data-index="${slotIndex}"]`);
                if (targetSlot && unitData) {
                    const cardEl = createBattlefieldCard(unitData, true);
        ...
        if (units[i]) {
            ...
            // Trigger play animation for newly deployed cards (player only, AI handled above)
            if (!hadCard && !isOpponent) {
                playPlayAnimation(cardEl);
            } else if (!hadCard && isOpponent && !isAITurn) {
                // Opponent card during player turn (e.g., from technique)
                playPlayAnimation(cardEl);
            }
...
function renderField(player, field) {
    ...
    // Check if this is a NEW AI field on FIRST iteration (when deploys happen)
    const isOpponent = player === 'opponent';
    const isNewField = !hadCard && field;
    const isFirstIteration = aiTurnIteration === 1;
    
    if (isNewField && isOpponent && isAITurn && isFirstIteration) {
        // Don't render yet - schedule it for later
        const delay = getNextAIActionDelay();
        const fieldData = field; // Capture field data NOW
        
        setTimeout(() => {
            // Find the slot again (in case DOM was updated)
            const targetSlot = document.querySelector(`[data-zone="${player}-field"]`);
            if (targetSlot && fieldData) {
                const cardEl = createCardElement(fieldData, false, true);
                cardEl.dataset.cardId = fieldData.id;
                targetSlot.innerHTML = '';
                targetSlot.appendChild(cardEl);
                ...
                setTimeout(() => {
                    playFieldAnimation(fieldData.id, false, 'opponent');
                }, 100);
                
                // Start field effects
                startFieldEffects(fieldData.id, 'opponent');
                previousFieldIds.opponent = fieldData.id;
            }
        }, delay);
    ...
    if (field) {
        ...
        if (player === 'opponent') {
            ...
            if (!hadCard || isReplacement) {
                // Trigger field animation for opponent! (only if not during AI turn - handled above)
                if (!isAITurn) {
                    setTimeout(() => {
...
function renderTraps(player, traps) {
    if (player === 'opponent') {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const hadCard = slot.querySelector('.card') !== null;
            const isNewTrap = !hadCard && i < traps;
            
            // If this is a NEW AI trap on FIRST iteration (when deploys happen), delay its appearance
            if (isNewTrap && isAITurn && isFirstIteration) {
                // Don't render yet - schedule it for later
                const delay = getNextAIActionDelay();
                const slotIndex = i; // Capture index NOW
                ...
                setTimeout(() => {
                    // Find the slot again (in case DOM was updated)
                    const targetSlot = document.querySelector(`[data-zone="opponent-trap"][data-index="${slotIndex}"]`);
                    if (targetSlot) {
                        const cardBack = document.createElement('div');
                        cardBack.className = 'card card-back';
                        ...
                        targetSlot.innerHTML = '';
                        targetSlot.appendChild(cardBack);
            ...
            height: 145px;
                ...
                // Play animation for newly set traps (non-AI turn)
                if (!hadCard && !isAITurn) {
                    playPlayAnimation(cardBack);
                }
...
async function advancePhase() {
    ...
    try {
        ...
        console.log('📦 ========================================');
        console.log('📦 RAW ADVANCE_PHASE RESPONSE:');
        console.log('📦 Keys in response:', Object.keys(data));
        console.log('📦 Has combat_log?', 'combat_log' in data);
        console.log('📦 Has log?', 'log' in data);
        
        if (data.log && data.log.length > 0) {
            console.log('📦 data.log[0] structure:', data.log[0]);
            console.log('📦 data.log[0] keys:', Object.keys(data.log[0]));
            console.log('📦 Full first log object:', JSON.stringify(data.log[0], null, 2));
        }
        console.log('📦 ========================================');
        
        // Check BOTH combat_log and log fields for combat messages
        const combatMessages = data.combat_log || data.log || [];
        
        if (combatMessages && Array.isArray(combatMessages) && combatMessages.length > 0 && gameState.active_player === 1) {
            console.log('⚔️ ========================================');
            console.log('⚔️ PARSING AI COMBAT MESSAGES!');
            console.log('⚔️ Number of messages:', combatMessages.length);
            console.log('⚔️ ========================================');
            
            // Track attacker/defender slots from "AI unit X (slot N)" and "considering attack" messages
            let attackerSlot = null;
            let defenderSlot = null;
            
            // Try to parse combat info from messages
            for (let index = 0; index < combatMessages.length; index++) {
                const logItem = combatMessages[index];
                
                // Extract message text from object
                let logMsg = logItem;
                if (typeof logItem === 'object' && logItem !== null) {
                    logMsg = logItem.message || logItem.text || logItem.msg || JSON.stringify(logItem);
                }
                
                if (typeof logMsg !== 'string') continue;
                
                console.log(`📦 [${index}] "${logMsg}"`);
                
                // Parse "AI unit X (slot N) looking for targets..."
                const lookingMatch = logMsg.match(/AI unit .* \(slot (\d+)\) looking for targets/);
                if (lookingMatch) {
                    attackerSlot = parseInt(lookingMatch[1]);
                    console.log(`   🎯 Found attacker slot: ${attackerSlot}`);
                }
                
                // Parse "AI considering attack: ... → opponent slot N"
                const consideringMatch = logMsg.match(/AI considering attack:.*opponent slot (\d+)/);
                if (consideringMatch) {
                    defenderSlot = parseInt(consideringMatch[1]);
                    console.log(`   🎯 Found defender slot: ${defenderSlot}`);
                }
                
                // Parse actual attack message "X attacks Y!"
                const attackMatch = logMsg.match(/^([^!]+) attacks ([^!]+)!$/);
                if (attackMatch && attackerSlot !== null && defenderSlot !== null) {
                    console.log(`   ⚔️ ATTACK DETECTED!`);
                    console.log(`      Attacker: slot ${attackerSlot}`);
                    console.log(`      Defender: slot ${defenderSlot}`);
                    
                    // Wait for UI to update
                    await new Promise(resolve => setTimeout(resolve, 500));
                    
                    const attackerZone = 'opponent-unit';
                    const defenderZone = 'your-unit';
                    
                    const attackerEl = document.querySelector(`[data-zone="${attackerZone}"][data-index="${attackerSlot}"] .card`);
                    const defenderEl = document.querySelector(`[data-zone="${defenderZone}"][data-index="${defenderSlot}"] .card`);
                    ...
                    if (attackerEl && defenderEl) {
                        ...
                        // Extract damage from next few messages
                        let damage = 0;
                        for (let j = index + 1; j < Math.min(index + 5, combatMessages.length); j++) {
                            const nextMsg = typeof combatMessages[j] === 'object' ? 
                                (combatMessages[j].message || combatMessages[j].text || '') : 
                                combatMessages[j];
                            const dmgMatch = nextMsg.match(/(\d+) ATK/);
                            if (dmgMatch) {
                                damage = parseInt(dmgMatch[1]);
                                console.log(`      💥 Found damage: ${damage}`);
                                break;
                            }
                        }
                        
                        if (damage === 0) damage = 1; // Default damage
                        
                        await playCombatAnimation(attackerEl, defenderEl, damage);
                        showDamageNumber(damage, defenderEl);
                        console.log('      ✅ Animation complete!');
                    } else {
                        console.log(`      ❌ Missing elements - attacker: ${!!attackerEl}, defender: ${!!defenderEl}`);
                    }
                ...
                // Add to log (use extracted message text)
                addLog(`⚔️ ${logMsg}`);
            }
        }
...
async function aiTurn() {
    ...
    isAITurn = true;  // Enable AI pacing system
    resetAIActionCounter();  // Reset action counter
    
    // Show AI loading overlay
    if (typeof showAILoading === 'function') {
        showAILoading('AI IS THINKING...', 'Planning strategy...');
    }
    
    // Add initial "AI is thinking" pause
    ...
    try {
        while (gameState.active_player === 1 && !gameState.winner) {
            ...
            iterations++;
            aiTurnIteration = iterations;  // Track current iteration for pacing system
            ...
            if (gameState.phase !== lastPhase) {
                ...
                // Update AI loading action based on phase
                if (typeof updateAILoadingAction === 'function') {
                    if (gameState.phase === 'deploy' || gameState.phase === 'start') {
                        updateAILoadingAction('Deploying cards...');
                    } else if (gameState.phase === 'combat') {
                        updateAILoadingAction('Declaring attacks...');
                    } else if (gameState.phase === 'end') {
                        updateAILoadingAction('Ending turn...');
                    }
                }
        ...
        // Add small pause before turn ends (makes transition feel smoother)
        await new Promise(resolve => setTimeout(resolve, 400));
    } finally {
        aiTurnInProgress = false;
        isAITurn = false;  // Disable AI pacing system
        resetAIActionCounter();  // Clean up counter
...
/* ═══════════════════════════════════════════════════════════════
   CORNER BADGE UI SYSTEM - Clean & Minimal
   ═══════════════════════════════════════════════════════════════ */
...
function updateLightningEnergy(player, currentEnergy, maxEnergy = 8) {
    const container = document.querySelector(`#${player}-lightning-bolts`);
    const numberDisplay = document.querySelector(`#${player}-energy-number`);
    
    if (!container) return;
    
    // Update energy number
    if (numberDisplay) {
        numberDisplay.textContent = currentEnergy;
    }
    
    // Clear existing
    container.innerHTML = '';
    
    // Create up to 8 lightning bolts (but game logic still caps at 5 normally)
    // This allows for edge cases where energy exceeds 5
    const displayBolts = Math.min(currentEnergy, maxEnergy);
    ...
    for (let i = 0; i < maxEnergy; i++) {
        ...
        if (i < currentEnergy) {
            bolt.classList.add('available');
        } else {
            bolt.classList.add('empty');
        }
...
// Animate energy gain
function animateLightningGain(player, amount) {
    const container = document.querySelector(`#${player}-lightning-bolts`);
    if (!container) return;
    
    const bolts = container.querySelectorAll('.lightning-bolt');
    let delay = 0;
    
    for (let i = 0; i < amount && i < bolts.length; i++) {
        setTimeout(() => {
            bolts[i].classList.remove('empty', 'used');
            bolts[i].classList.add('available', 'gaining');
            
            setTimeout(() => {
                bolts[i].classList.remove('gaining');
            }, 500);
        }, delay);
        delay += 80;
    }
    ...
    // Show notification
    showActionNotification('+' + amount + ' ⚡', player, 'energy-gain');
}

// Animate energy spend
function animateLightningSpend(player, amount, currentEnergy) {
    const container = document.querySelector(`#${player}-lightning-bolts`);
    if (!container) return;
    
    const bolts = container.querySelectorAll('.lightning-bolt');
    
    for (let i = currentEnergy; i < currentEnergy + amount && i < bolts.length; i++) {
        bolts[i].classList.remove('available');
        bolts[i].classList.add('used');
    }
    
    showActionNotification('-' + amount + ' ⚡', player, 'energy-spend');
}
...
function updateTurnLossDisplay(player, turnLoss, maxLoss = 3) {
    const display = document.querySelector(`#${player}-turn-loss`);
    const value = document.querySelector(`#${player}-turn-loss-value`);
    
    if (!display || !value) return;
    
    // Update value
    value.textContent = turnLoss + '/' + maxLoss;
    
    // Update danger state
    display.classList.remove('danger-safe', 'danger-warning', 'danger-critical');
    
    if (turnLoss === 0) {
        display.classList.add('danger-safe');
    } else if (turnLoss === 1) {
        display.classList.add('danger-warning');
    } else if (turnLoss >= 2) {
        display.classList.add('danger-critical');
    }
}

// Animate turn loss gain (screen shake + notification)
function animateTurnLossGain(player, newLoss) {
    updateTurnLossDisplay(player, newLoss);
    ...
    showActionNotification('TURN LOSS!', player, 'turn-loss');
}
...
function updatePhaseIndicatorCompact(phase) {
    const indicator = document.getElementById('phase-indicator-compact');
    if (!indicator) return;
    
    const icon = indicator.querySelector('.phase-icon-compact');
    const text = indicator.querySelector('.phase-text-compact');
    
    // Remove all phase classes
    indicator.classList.remove('phase-deploy', 'phase-combat', 'phase-end');
    
    // Update based on phase
    switch(phase) {
        case 'deploy':
        case 'start':
            indicator.classList.add('phase-deploy');
            icon.textContent = '🔧';
        ...
        case 'combat':
            ...
            break;
        case 'end':
            indicator.classList.add('phase-end');
            icon.textContent = '✓';
...
function updateTurnCounterBadge(turnNumber) {
    const counter = document.getElementById('turn-number-badge');
    if (counter) {
        counter.textContent = turnNumber;
    }
    
    // Also update old counter for compatibility
    const oldCounter = document.getElementById('turn-number');
    if (oldCounter) {
        oldCounter.textContent = turnNumber;
    }
}
...
function updateActivePlayerBadge(activePlayer) {
    const yourBadge = document.getElementById('corner-badge-player');
    const opponentBadge = document.getElementById('corner-badge-opponent');
    
    if (!yourBadge || !opponentBadge) return;
    
    // Remove active states
    yourBadge.classList.remove('active-turn', 'ai-thinking');
    opponentBadge.classList.remove('active-turn', 'ai-thinking');
    ...
    if (activePlayer === 0) {
        ...
        // AI turn
        opponentBadge.classList.add('active-turn');
        if (isAITurn) {
            opponentBadge.classList.add('ai-thinking');
        }
    }
}
...
function updateBadgeDeckCount(player, deckSize) {
    const counter = document.getElementById(`${player}-badge-deck`);
    if (counter) {
        counter.textContent = deckSize;
    }
}

function updateBadgeHandCount(player, handSize) {
    const counter = document.getElementById(`${player}-badge-hand`);
    if (counter) {
        counter.textContent = handSize;
    }
}
...
function showActionNotification(message, player, type) {
    ...
    const badge = document.getElementById(`corner-badge-${player === 'your' ? 'player' : 'opponent'}`);
    if (badge) {
...
function updateCornerBadgeUI() {
    if (!gameState) return;
    
    // Update energy
    updateLightningEnergy('your', gameState.you.energy);
    updateLightningEnergy('opponent', gameState.opponent.energy);
    
    // Update turn loss
    updateTurnLossDisplay('your', gameState.you.control_loss);
    updateTurnLossDisplay('opponent', gameState.opponent.control_loss);
    
    // Update phase
    updatePhaseIndicatorCompact(gameState.phase);
    
    // Update turn
    updateTurnCounterBadge(gameState.turn);
    
    // Update active player
    updateActivePlayerBadge(gameState.active_player);
    
    // Update deck/hand counts
    updateBadgeDeckCount('your', gameState.you.deck_count);
    updateBadgeDeckCount('opponent', gameState.opponent.deck_count);
    updateBadgeHandCount('your', gameState.you.hand.length);
    updateBadgeHandCount('opponent', gameState.opponent.hand_count);
}
