...
function renderBattlefield(player, units) {
    for (let i = 0; i < 5; i++) {
        ...
        if (units[i]) {
            ...
            // Trigger slide-in animation for newly deployed cards
...
async function declareAttack(attackerIndex, defenderIndex) {
    ...
    try {
        ...
        if (data.result.error) {
            ...
            const attackerEl = document.querySelector(`[data-zone="your-battlefield"][data-index="${attackerIndex}"] .card`);
            const defenderEl = document.querySelector(`[data-zone="opponent-battlefield"][data-index="${defenderIndex}"] .card`);
            
            // Play attack animation if elements exist
            if (attackerEl && defenderEl && data.result.combat_log) {
                // Extract damage from combat log (look for "deals X damage")
                const damageMatch = data.result.combat_log.join(' ').match(/deals (\d+) damage/);
