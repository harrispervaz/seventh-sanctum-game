...
function renderField(player, field) {
    ...
    if (field) {
        ...
        // Don't trigger animations during initial load - only for YOUR field
        if (!hadCard && player === 'your') {
            // New field played - will be animated via playSelectedCard
        } else if (hadCard && oldFieldId !== field.id && player === 'your') {
            // Field replaced - animate (handled separately)
        }
        
        slot.appendChild(cardEl);
        
        // Start persistent effects for YOUR field
        if (player === 'your' && field.id !== activeFieldEffects) {
            startFieldEffects(field.id);
        }
    } else {
...
function playFieldAnimation(fieldCardId, isReplacement = false) {
    return new Promise((resolve) => {
        ...
        // Wait for shatter if replacing
        setTimeout(() => {
            // Create ripples (3 rings)
            for (let i = 0; i < 3; i++) {
            ...
            setTimeout(() => {
                const newCard = fieldSlot.querySelector('.card');
                if (newCard) {
...
function startFieldEffects(fieldId) {
    stopFieldEffects(); // Clear any existing
    ...
    switch(fieldId) {
        case 'skyforge_assembly_line':
            createAssemblyLineEffect(overlay);
            break;
        case 'skyforge_relay_node':
            createRelayNodeEffect(overlay);
            break;
        case 'skyforge_kill_zone':
            createKillZoneEffect(overlay);
            break;
        case 'skyforge_rustfields':
            createRustfieldsEffect(overlay);
            break;
        case 'miasma_lowlands_mist':
            createLowlandsMistEffect(overlay);
            break;
        case 'miasma_blight_pools':
            createBlightPoolsEffect(overlay);
            break;
        case 'miasma_rotfall_expanse':
            createRotfallExpanseEffect(overlay);
            break;
    }
