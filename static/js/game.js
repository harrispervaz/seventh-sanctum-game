    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (units[i]) {
            ...
            if (previousState && !previousState.is_exhausted && units[i].is_exhausted) {
                ...
                }, 400); // Small delay to let rotation start
            }
...
function createBattlefieldCard(card, isOpponent = false) {
    ...
    if (card.is_exhausted) {
        ...
        cardEl.style.transition = 'transform 0.4s cubic-bezier(0.34, 1.56, 0.64, 1), opacity 0.3s ease, filter 0.3s ease';
            ...
            data.combat_log.forEach(async (logMsg) => {
                ...
                if (attackMatch && damageMatch) {
                    ...
                    if (attackerEl && defenderEl && damage > 0) {
                        console.log(`🎬 Playing AI combat animation: attacker ${attackerIndex} → defender ${defenderIndex}, damage ${damage}`);
