...
function renderBattlefield(player, units) {
    for (let i = 0; i < 5; i++) {
        ...
        if (units[i]) {
            ...
            // Trigger play animation for newly deployed cards
            if (!hadCard) {
                // Add staggered delay for AI card plays
...
function renderField(player, field) {
    ...
    if (field) {
        ...
        const isNewField = !hadCard;
        if (isNewField) {
            // Add staggered delay for AI field plays
            if (player === 'opponent' && isAITurn) {
                const delay = getNextAIActionDelay();
                setTimeout(() => {
        ...
        if (player === 'opponent') {
            ...
            if (isNewField || isReplacement) {
                ...
                const animationDelay = isAITurn ? getNextAIActionDelay() : 100;
                setTimeout(() => {
    ...
    if (player === 'opponent') {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            height: 145px;
                ...
                // Play animation for newly set traps with stagger for AI
                if (!hadCard) {
                    if (isAITurn) {
                        const delay = getNextAIActionDelay();
                        setTimeout(() => {
