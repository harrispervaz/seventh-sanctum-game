...
async function advancePhase() {
    ...
    try {
        ...
        if (data.combat_log) {
            console.log('📦 data.combat_log:', data.combat_log);
        }
        console.log('📦 ========================================');
        ...
        if (combatMessages && Array.isArray(combatMessages) && combatMessages.length > 0) {
            ...
            combatMessages.forEach((msg, i) => {
                console.log(`⚔️ [${i}] "${msg}"`);
            });
            console.log('⚔️ ========================================');
            
            // Try to parse combat info from messages
            combatMessages.forEach(async (logMsg, index) => {
                if (typeof logMsg !== 'string') {
                    console.log(`⚠️ Message ${index} is not a string:`, logMsg);
                    return;
                }
                
                console.log(`🔍 Parsing message ${index}:`);
                console.log(`   Raw: "${logMsg}"`);
                ...
                const patterns = [
                    ...
                    { name: 'Pattern 4', regex: /^([^\s]+).*attacks.*([^\s]+)/ }, // Generic
                ];
                ...
                console.log('   Best attack match:', attackMatch);
