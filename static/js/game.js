...
function selectCard(card, index) {
    ...
    // Different message for traps (FREE to set)
    if (card.type === 'TRAP') {
        addLog(`Selected: ${card.name} - Click "Set Trap" (FREE)`);
    } else {
    ...
    // Highlight available trap slots if selecting a trap
    highlightAvailableTrapSlots();
...
function startFieldEffects(fieldId, player = 'your') {
    // Add trail to field card
    const fieldSlot = document.querySelector(`[data-zone="${player}-field"]`);
    if (fieldSlot) {
        const card = fieldSlot.querySelector('.card');
        if (card) {
            // Determine faction
            let factionClass = 'field-card-generic';
            if (fieldId.startsWith('skyforge_')) {
                factionClass = 'field-card-skyforge';
            } else if (fieldId.startsWith('miasma_')) {
                factionClass = 'field-card-miasma';
            }
            
            card.classList.add('field-card-active', factionClass);
        }
    }
    
    // Update background tint based on YOUR field only
    if (player === 'your') {
        updateBackgroundTint(fieldId);
    }
}

// Stop field visual effects
function stopFieldEffects(player = 'your') {
    // Remove trail from field card
    const fieldSlot = document.querySelector(`[data-zone="${player}-field"]`);
    if (fieldSlot) {
        const card = fieldSlot.querySelector('.card');
        if (card) {
            card.classList.remove('field-card-active', 'field-card-skyforge', 'field-card-miasma', 'field-card-generic');
        }
    }
    
    // Reset background if YOUR field removed
    if (player === 'your') {
        updateBackgroundTint(null);
    }
}

// Update background tint based on field type
function updateBackgroundTint(fieldId) {
    const body = document.body;
    
    // Remove all field background classes
    body.classList.remove('field-bg-skyforge', 'field-bg-miasma', 'field-bg-generic');
    
    if (!fieldId) return; // No field active
    
    // Add appropriate background class
    if (fieldId.startsWith('skyforge_')) {
        body.classList.add('field-bg-skyforge');
    } else if (fieldId.startsWith('miasma_')) {
        body.classList.add('field-bg-miasma');
    } else {
        body.classList.add('field-bg-generic');
    }
}
...
// ============================================================
// PHASE 2: TRAP DEPLOYMENT SYSTEM
// ============================================================

// Highlight available trap slots when a trap card is selected
function highlightAvailableTrapSlots() {
    ...
    // If a trap is selected, highlight empty slots
    if (selectedCard && selectedCard.card.type === 'TRAP') {
        document.querySelectorAll('[data-zone="your-trap"]').forEach(slot => {
            // Check if slot is empty (no card present)
