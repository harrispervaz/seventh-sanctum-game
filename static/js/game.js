...
async function declareAttack(attackerIndex, defenderIndex) {
    ...
    try {
        ...
        console.log('🎭 DEBUG: Attack response received:', data);
        console.log('🎭 DEBUG: data.result:', data.result);
        console.log('🎭 DEBUG: data.result.trap_trigger:', data.result.trap_trigger);
        
        // ============================================================
        // PHASE 3B: CHECK FOR TRAP TRIGGER
        // ============================================================
        if (data.result.trap_trigger) {
            console.log('🎭 DEBUG: TRAP TRIGGER DETECTED!');
            // Opponent has a trap that can activate!
            addLog(`🎭 ${data.result.trigger_message}`);
            
            // Show trap activation prompt to opponent (player 1)
            // Store pending attack data so we can resume after trap resolution
            pendingAttackData = data.result.pending_attack;
            
            // Show the trap activation prompt
            showTrapActivationPromptWithHandler(
                data.result.trap,
                data.result.trap_slot,
                data.result.trigger_message,
                1  // Opponent is player 1
            );
            
            return; // Don't continue with attack until trap is resolved
        }
        
        console.log('🎭 DEBUG: No trap trigger, continuing with normal attack');
...
// TEMPORARY TEST BUTTON - Phase 3A
document.getElementById('test-trap-btn').addEventListener('click', testTrapPrompt);
...
function showTrapActivationPromptWithHandler(trap, trapSlot, triggerMessage, player) {
    const modal = document.createElement('div');
...
async function handleTrapActivation(trap, trapSlot, activate, player) {
    try {
        ...
        if (activate) {
            ...
            // TODO: Phase 3C - Play flip animation
        ...
        // TODO: Continue with pending attack if there was one
