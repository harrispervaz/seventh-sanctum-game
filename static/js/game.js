...
function renderBattlefield(player, units) {
    for (let i = 0; i < 5; i++) {
        ...
        if (isNewCard && isOpponent && isAITurn) {
            ...
            setTimeout(() => {
                // Re-check that card should still be rendered
                const currentUnit = gameState.battlefield[player === 'your' ? 0 : 1][i];
                if (currentUnit) {
                    const cardEl = createBattlefieldCard(currentUnit, true);
                    slot.innerHTML = '';
...
function renderField(player, field) {
    ...
    if (isNewField && isOpponent && isAITurn) {
        ...
        setTimeout(() => {
            // Re-check that field should still be rendered
            const currentField = gameState.battlefield[player === 'your' ? 0 : 1].field;
            if (currentField && currentField.id === field.id) {
                const cardEl = createCardElement(currentField, false, true);
                cardEl.dataset.cardId = currentField.id;
                slot.innerHTML = '';
                slot.appendChild(cardEl);
                ...
                setTimeout(() => {
                    playFieldAnimation(currentField.id, false, 'opponent');
                }, 100);
                
                // Start field effects
                startFieldEffects(currentField.id, 'opponent');
                previousFieldIds.opponent = currentField.id;
            }
        }, delay);
    ...
    if (player === 'opponent') {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (isNewTrap && isAITurn) {
                ...
                setTimeout(() => {
                    // Re-check that trap should still be rendered
                    const currentTraps = gameState.battlefield[1].traps;
                    if (i < currentTraps) {
                        const cardBack = document.createElement('div');
                        cardBack.className = 'card card-back';
