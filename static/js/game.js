...
function renderBattlefield(player, units) {
    for (let i = 0; i < 5; i++) {
        ...
        // If this is a NEW AI card during DEPLOY phase, delay its appearance
        const isOpponent = player === 'opponent';
        const isNewCard = !hadCard && units[i];
        const isDeployPhase = gameState?.phase === 'deploy';
        
        // DEBUG LOGGING
        if (isNewCard && isOpponent && isAITurn) {
            console.log(`🔍 New AI card in slot ${i}:`);
            console.log(`   isAITurn: ${isAITurn}`);
            console.log(`   gameState.phase: ${gameState?.phase}`);
            console.log(`   isDeployPhase: ${isDeployPhase}`);
            console.log(`   Will delay: ${isDeployPhase}`);
        }
        
        if (isNewCard && isOpponent && isAITurn && isDeployPhase) {
...
function renderField(player, field) {
    ...
    // Check if this is a NEW AI field during DEPLOY phase
    const isOpponent = player === 'opponent';
    const isNewField = !hadCard && field;
    const isDeployPhase = gameState?.phase === 'deploy';
    
    if (isNewField && isOpponent && isAITurn && isDeployPhase) {
        // Don't render yet - schedule it for later
        const delay = getNextAIActionDelay();
...
function renderTraps(player, traps) {
    if (player === 'opponent') {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            // If this is a NEW AI trap during DEPLOY phase, delay its appearance
            if (isNewTrap && isAITurn && isDeployPhase) {
                // Don't render yet - schedule it for later
                const delay = getNextAIActionDelay();
                const slotIndex = i; // Capture index NOW
