...
async function playSelectedCard() {
    ...
    try {
        ...
        if (data.result.error) {
            ...
            // Update UI after trap resolution
            updateUI();
...
async function advancePhase() {
    try {
        ...
        // PHASE 3D BATCH 3: Check for Flute of Slumber trigger
        if (data.ready_trap_trigger) {
        ...
        // PHASE 3D BATCH 3: Check for Counter-Sigil trigger
...
async function aiTurn() {
    ...
    while (gameState.active_player === 1 && !gameState.winner) {
        ...
        // Check if a trap triggered during AI's turn that needs human response
        if (gameState.ready_trap_trigger || gameState.deployment_trap_trigger || 
            gameState.field_trap_trigger || gameState.technique_trap_trigger ||
            gameState.counter_sigil_trigger) {
            // Trap triggered - stop AI turn loop, human needs to respond
            return;
        }
...
async function handleTrapPrompt(trapOwner, trapSlot, trap, triggerData, trapType, techniqueInLimbo = null) {
    if (trapOwner === 1) {
        ...
        // Use the existing trap activation modal
        showTrapActivationPromptWithHandler(
            trap,
            trapSlot,
            triggerMessage,
...
function getTrapTriggerMessage(trap, triggerData, trapType) {
    if (trapType === 'deployment') {
        return `Your ${trap.name} triggered! An enemy unit was deployed.`;
    ...
    try {
        ...
        if (activate) {
            ...
            if (data.effect_result) {
                ...
                // Attack was NOT cancelled - continue with pending attack
                if (pendingAttackData) {
