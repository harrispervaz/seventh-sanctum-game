...
async function handleTrapPrompt(trapOwner, trapSlot, trap, triggerData, trapType, techniqueInLimbo = null) {
    ...
    if (trapOwner === 1) {
        ...
        // For ready traps: Check whose units are readying
        //   - If readied_player === 0 (human units), it's start of AI's turn → wasAITurn = true
        //   - If readied_player === 1 (AI units), it's start of human's turn → wasAITurn = false
        // For other traps: Check current active player
        let wasAITurn;
        if (trapType === 'ready') {
            // Ready traps trigger at start of turn when opponent's units ready
            // readied_player = 0 means player units readying = start of AI turn
            // readied_player = 1 means AI units readying = start of player turn
            wasAITurn = (triggerData?.readied_player === 0);
        } else {
