...
async function advancePhase() {
    try {
        ...
        // NOTE: AI turn is now handled by the aiTurn() loop function
        // We don't call aiTurn here anymore to prevent duplicate calls
...
async function aiTurn() {
    ...
    try {
        while (gameState.active_player === 1 && !gameState.winner) {
            ...
            iterations++;
            console.log(`🔄 AI turn iteration ${iterations}/${MAX_ITERATIONS}`);
            
            if (iterations > MAX_ITERATIONS) {
                console.error('🚨 CRITICAL: AI turn exceeded maximum iterations!');
                console.error('Current gameState:', gameState);
            ...
            try {
                ...
                console.log('   Current active_player:', gameState?.active_player);
            } catch (error) {
