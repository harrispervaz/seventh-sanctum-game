...
function renderField(player, field) {
    ...
    if (field) {
        ...
        // Trigger slide-in animation for newly played fields
        if (!hadCard) {
            playSlideInAnimation(cardEl);
        }
...
async function playSelectedCard() {
    ...
    try {
        ...
        if (data.result.error) {
            ...
            // Technique resolved - fire particles!
            if (selectedCard.card.type === 'TECHNIQUE') {
                playTechniqueEffect(selectedCard.card.id);
            }
            addLog(`✅ ${data.result.message}`);
            gameState = data.state;
            updateUI();
...
async function applyTargetedTechnique(targetPlayer, targetIndex) {
    ...
    try {
        ...
        if (data.result.error) {
            ...
            // Fire particles for targeted technique!
            playTechniqueEffect(targetingCard.card_id);
            addLog(`✅ ${data.result.message}`);
            gameState = data.state;
        }
...
const TECHNIQUE_PARTICLES = {
    ...
    'skyforge_emergency_repairs': { color: '#6db465', shape: '🔧', count: 10, spread: 100, type: 'sparkle' },
    ...
    'skyforge_counter_measure':   { color: '#4a90e2', shape: '🛡️', count: 8,  spread: 80,  type: 'sparkle' },
    'skyforge_self_destruct':     { color: '#ff4444', shape: '💥', count: 30, spread: 250, type: 'explosion' },
    'skyforge_decoy_protocol':    { color: '#aaaaaa', shape: '👻', count: 10, spread: 120, type: 'sparkle' },
    'skyforge_lockdown':          { color: '#ff6600', shape: '🔒', count: 12, spread: 100, type: 'sparkle' },

    // Miasma - Purple/Green toxic
    ...
    'miasma_foglash':             { color: '#6a3568', shape: '💀', count: 20, spread: 180, type: 'explosion' },
    'miasma_miasma_potion':       { color: '#9b59b6', shape: '🧪', count: 12, spread: 120, type: 'sparkle' },
    'miasma_rot_beneath_the_surface': { color: '#3d5a00', shape: '🌑', count: 15, spread: 150, type: 'fog' },
    ...
    'generic_counter_sigil':      { color: '#4a90e2', shape: '🔵', count: 12, spread: 120, type: 'burst' },
    'generic_earthquake':         { color: '#8b6914', shape: '🌋', count: 25, spread: 220, type: 'explosion' },
    'generic_false_step':         { color: '#888888', shape: '👣', count: 10, spread: 100, type: 'sparkle' },
    'generic_flute_of_slumber':   { color: '#c8a2c8', shape: '💤', count: 15, spread: 150, type: 'fog' },
    'generic_return_to_sender':   { color: '#00d4ff', shape: '↩️', count: 12, spread: 120, type: 'burst' },
};
...
function playTechniqueEffect(cardId) {
    const config = TECHNIQUE_PARTICLES[cardId] || DEFAULT_PARTICLES;
    const center = getScreenCenter();
    
    // Flash screen with technique color
    ...
    for (let i = 0; i < config.count; i++) {
        createParticle(center.x, center.y, config);
    }
    
    // Extra ring of particles for burst type
    if (config.type === 'burst' || config.type === 'explosion') {
        setTimeout(() => {
            for (let i = 0; i < Math.floor(config.count / 2); i++) {
                createParticle(center.x, center.y, { ...config, spread: config.spread * 1.5 });
            }
        }, 150);
    }
    
    // Slow fog drift for fog type
    if (config.type === 'fog') {
        setTimeout(() => {
            for (let i = 0; i < 8; i++) {
                createParticle(center.x, center.y, { ...config, spread: config.spread * 0.5 });
            }
        }, 300);
    }
}
