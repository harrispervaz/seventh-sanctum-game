...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Show deck selection immediately
    if (deckModal) {
    ...
    if (skyforgeBtn) {
        skyforgeBtn.addEventListener('click', () => {
            console.log('Selected Skyforge deck');
    ...
    if (miasmaBtn) {
        miasmaBtn.addEventListener('click', () => {
            console.log('Selected Miasma deck');
            deckModal.classList.remove('active');
            startGame('miasma');
        });
    }
...
function startGame(playerFaction) {
    // The opponent always plays the opposite faction for now
    const opponentFaction = playerFaction === 'skyforge' ? 'miasma' : 'skyforge';
    
    console.log(`Starting game: You (${playerFaction}) vs AI (${opponentFaction})`);
    
    // Create new game with faction choice
    fetch('/api/game/new', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
