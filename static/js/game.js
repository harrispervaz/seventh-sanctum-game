...
async function playSelectedCard() {
    ...
    try {
        ...
        if (data.result.error) {
            ...
            // DON'T show prompt yet - let AI finish deploying all units first
            addLog(`🎭 ${data.result.trap.name} triggered - collecting targets...`);
            
            // Just update the state and continue AI turn
            // The trap will be shown when AI turn ends via advance_phase
            gameState = data.result.state || gameState;
            updateUI();
            // Continue AI turn - don't await trap prompt yet
...
async function advancePhase() {
    try {
        ...
        gameState = data.state || data;  // Fallback to data if state property missing
        
        // Validate state before updating UI
        if (!gameState.phase || !gameState.you || !gameState.opponent) {
            console.error('❌ Invalid state received from advance_phase:', gameState);
            addLog('❌ Game state error - please refresh the page');
            return;
        }
        ...
        addLog('❌ Error advancing game - please refresh the page');
...
async function aiTurn() {
    ...
    while (gameState.active_player === 1 && !gameState.winner) {
        ...
        if (iterations > MAX_ITERATIONS) {
            console.error('🚨 AI turn exceeded maximum iterations - breaking loop!');
            addLog('⚠️ AI turn error - please refresh the page');
            break;
        }
        
        await new Promise(resolve => setTimeout(resolve, 800)); // Slight delay so we can see AI actions
        
        // Don't advance if a trap modal is currently being shown to the human
        if (trapModalActive) {
            console.log('⏸️ AI turn paused - trap modal active');
            break;  // Exit the loop, will resume after trap is resolved
        }
        
        try {
            await advancePhase();
            // advancePhase() handles all trap triggers internally
            // It will pause AI turn until trap is resolved
        } catch (error) {
            console.error('❌ Error in AI turn:', error);
            addLog('⚠️ AI turn error - please refresh the page');
            break;  // Exit loop on error to prevent infinite requests
        }
...
async function handleTrapPrompt(trapOwner, trapSlot, trap, triggerData, trapType, techniqueInLimbo = null) {
    if (trapOwner === 1) {
        ...
        // Store the AI turn state so we can resume after trap resolution
        const wasAITurn = gameState.active_player === 1;
...
function showTrapActivationPromptWithHandler(trap, trapSlot, triggerMessage, player, triggerData = null, wasAITurn = false) {
    ...
    document.getElementById('trap-activate-yes').addEventListener('click', async () => {
        try {
            ...
            if (wasAITurn && gameState.active_player === 1) {
    ...
    document.getElementById('trap-activate-no').addEventListener('click', async () => {
        try {
            ...
            // Resume AI turn if needed
            if (wasAITurn && gameState.active_player === 1) {
                console.log('🤖 Resuming AI turn after declining trap...');
                setTimeout(aiTurn, 500);
            }
