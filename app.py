...
class GameState:
    ...
    def get_card_data(self, card_id, is_exhausted=False, wither_stacks=0, is_corrupt=False):
        ...
        # Calculate actual DEF with Wither (minimum 1)
        if card.get('def') is not None:
            card['def_actual'] = max(1, card['def'] - wither_stacks)
        return card
    ...
    def attack(self, attacker_player, attacker_index, defender_index):
        ...
        if not defender_destroyed:
            ...
            if not defender_player_obj['battlefield_exhausted'][defender_index]:
