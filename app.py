...
class GameState:
    ...
    def resolve_trap_effect(self, trap_id, trap_owner, trigger_data=None):
        ...
        # ============================================
            ...
            # Exhaust deployed unit + disable abilities (Corrupt) until start of trap owner's next turn
            ...
            if deployed_player is not None and deployed_index is not None:
                ...
                # Exhaust the unit (cannot attack)
    ...
    def start_turn(self):
        ...
        if flute_triggered_units:
            # Store FIRST triggered unit (handle one at a time)
            first_trigger = flute_triggered_units[0]
            self.pending_trap_trigger = {
                'trap_type': 'ready_trap_trigger',
                'trap': first_trigger['trap_info']['trap'],
                'trap_slot': first_trigger['trap_info']['slot'],
                'trigger_message': first_trigger['trap_info']['trigger_message'],
                'trigger_data': {
                    'readied_player': self.active_player,
                    'readied_index': first_trigger['unit_index'],
                    'unit_name': first_trigger['unit_name']
                },
                'trap_owner': first_trigger['trap_owner']
            }
