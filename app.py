...
class GameState:
    ...
    def end_turn(self):
        ...
        # Check hand size limit (max 7 cards) - mark for discard but don't block
        if len(current_player['hand']) > 7:
            over_limit = len(current_player['hand']) - 7
            self.log(f"⚠️ Player {self.active_player + 1} has {len(current_player['hand'])} cards! Must discard {over_limit} at end of turn.")
            current_player['must_discard'] = over_limit
            # Don't return - let turn end, but player must discard before next turn
    ...
    def start_turn(self):
        ...
        # Check if must discard from previous turn
        if current_player.get('must_discard', 0) > 0:
            self.log(f"⚠️ Must discard {current_player['must_discard']} cards before continuing!")
            return  # Don't proceed with turn until discarded
...
def discard_card(game_id):
    ...
    if current_player['must_discard'] == 0:
    ...
    game.log(f"Discarded {CARDS_BY_ID[discarded_card]['name']} (hand limit)")
    
    # If finished discarding, continue with start of turn
    if current_player['must_discard'] == 0:
        game.log(f"Hand reduced to 7 cards.")
        # Now run the actual start_turn logic
        game.start_turn()
