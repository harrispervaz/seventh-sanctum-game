...
class GameState:
    ...
    def resolve_trap_effect(self, trap_id, trap_owner, trigger_data=None):
        ...
        # ============================================
            ...
            # Use selected target if provided, otherwise fall back to deployed_index (for AI auto-activation)
            target_index = trigger_data.get('selected_target_index') or trigger_data.get('deployed_index') or trigger_data.get('recently_deployed_index')
            
            if deployed_player is not None and target_index is not None:
                unit_card_id = self.players[deployed_player]['battlefield'][target_index]
                ...
                # Exhaust the unit
                self.players[deployed_player]['battlefield_exhausted'][target_index] = True
                
                # Apply Corrupt (abilities disabled)
                self.players[deployed_player]['battlefield_corrupt'][target_index] = True
                self.players[deployed_player]['battlefield_corrupt_applied_turn'][target_index] = self.half_turn
                
                # Apply no_attack flag (cannot attack until start of trap owner's next turn)
                self.players[deployed_player]['battlefield_no_attack'][target_index] = True
            ...
            if deployed_player is not None and target_index is not None:
                ...
                self.players[deployed_player]['battlefield_wither'][target_index] += 1
                self.players[deployed_player]['battlefield_wither_applied_turn'][target_index] = self.half_turn
        ...
        if card['type'] == 'UNIT':
            ...
            for i in range(5):
                if player['battlefield'][i] is None:
                    ...
                    if deployment_traps:
                        # Collect all enemy units as available targets
                        available_targets = []
                        ...
                        self.pending_trap_trigger = {
                            ...
                            'trigger_data': {
                                'deployed_player': player_idx,
                                'recently_deployed_index': i,  # The unit that just triggered it
                                'recently_deployed_name': card['name'],
                                'available_targets': available_targets  # All enemy units
                            },
