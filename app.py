...
class GameState:
    ...
    def resolve_trap_effect(self, trap_id, trap_owner, trigger_data=None):
        ...
        # ============================================
            ...
            deployed_index = trigger_data.get('deployed_index')
            
            if deployed_player is not None and deployed_index is not None:
                unit_card_id = self.players[deployed_player]['battlefield'][deployed_index]
                unit_name = CARDS_BY_ID[unit_card_id]['name']
                
                # Exhaust the unit
                self.players[deployed_player]['battlefield_exhausted'][deployed_index] = True
                
                # Apply Corrupt (abilities disabled)
                self.players[deployed_player]['battlefield_corrupt'][deployed_index] = True
                self.players[deployed_player]['battlefield_corrupt_applied_turn'][deployed_index] = self.half_turn
                
                # Apply no_attack flag (cannot attack until start of trap owner's next turn)
                self.players[deployed_player]['battlefield_no_attack'][deployed_index] = True
                
                result['messages'].append(f"{unit_name} locked down! (Exhausted + Corrupted + Cannot attack)")
                self.log(f"{unit_name} locked down")
        
                # Set no_attack flag (cannot attack even after ready)
                self.players[deployed_player]['battlefield_no_attack'][deployed_index] = True
                
                result['messages'].append(f"{unit_name} locked down! Cannot attack or use abilities!")
            ...
            if deployed_player is not None and deployed_index is not None:
                ...
                self.players[deployed_player]['battlefield_wither'][deployed_index] += 1
                self.players[deployed_player]['battlefield_wither_applied_turn'][deployed_index] = self.half_turn
