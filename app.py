...
class GameState:
    ...
    def end_turn(self):
        ...
        # Check hand size limit (max 7 cards)
        if len(current_player['hand']) > 7:
            self.log(f"⚠️ Player {self.active_player + 1} has {len(current_player['hand'])} cards! Must discard to 7.")
            # Set flag for discard phase
            current_player['must_discard'] = len(current_player['hand']) - 7
            return  # Wait for player to discard before ending turn
...
def discard_card(game_id):
    ...
    # If finished discarding, end turn properly
    if current_player['must_discard'] == 0:
        game.log(f"Hand reduced to 7 cards. Ending turn.")
        # Switch active player
