        ...
        if card['type'] == 'UNIT':
            ...
            for i in range(5):
                if player['battlefield'][i] is None:
                    ...
                    if deployment_traps:
                        # Return trap trigger (pause deployment)
                        return {
...
def activate_trap(game_id):
    ...
    if activate:
        ...
        # PHASE 3D BATCH 3: Handle Flute of Slumber resolution
        if trap_id == 'generic_flute_of_slumber' and hasattr(game, 'pending_trap_trigger'):
            # Clear pending trap trigger
            game.pending_trap_trigger = None
            # Unit stays exhausted - don't ready it
            game.log("✅ Flute resolved - unit remains exhausted")
        ...
        # PHASE 3D BATCH 3: If Flute wasn't activated, ready the unit
        if trap_id == 'generic_flute_of_slumber' and hasattr(game, 'pending_trap_trigger'):
            ...
            if readied_player is not None and readied_index is not None:
                # Ready the unit
                game.players[readied_player]['battlefield_exhausted'][readied_index] = False
                unit_name = trigger_data.get('unit_name', 'Unit')
                game.log(f"{unit_name} becomes ready")
