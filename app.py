...
class GameState:
    ...
    def attack(self, attacker_player, attacker_index, defender_index):
        ...
        if attacker_atk > defender_def_actual:
            ...
            # Kill Zone: attacker's player has Kill Zone field + attacker is Skyforge faction
            ...
            for i in range(5):
                if player['battlefield'][i] is None:
                    ...
                    if opponent.get('field') == 'miasma_blight_pools':
                        ...
                        player['battlefield_wither_applied_turn'][i] = self.turn
    ...
    def end_turn(self):
        ...
        for p_idx in range(2):
            ...
            # Check if Rotfall Expanse is on the field (either player can control it)
            either_has_rotfall = (
            ...
            if either_has_rotfall:
                ...
                if unit_count > 3:
                    ...
                    self.log(f"🌑 Rotfall Expanse: Player {p_idx + 1} has {unit_count} Units - must destroy {must_destroy}!")
                    if p_idx == 0:
                        return  # Block turn end - frontend will show Rotfall modal
    ...
    def start_turn(self):
        ...
        # Relay Node: gain 1⚡ if you have a Skyforge Unit deployed (max 1 per turn)
        ...
        if current_player.get('field') == 'miasma_lowlands_mist':
            for i in range(5):
                if opponent['battlefield'][i] is not None:
                    ...
                    self.log(f"🌫️ Lowlands Mist: {unit_name} withers (DEF -1 this turn)")
