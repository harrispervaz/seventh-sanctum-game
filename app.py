...
class GameState:
    ...
    def end_turn(self):
        ...
        # Wither expires at the START of the afflicted unit's controller's turn
        # We use half_turn (increments every time any player starts a turn) for precise tracking
        for i in range(5):
            if current_player['battlefield'][i] is not None:
                wither_turn = current_player['battlefield_wither_applied_turn'][i]
                # Wither expires if applied_half_turn < current half_turn
                # i.e. it was applied at least one full half-turn ago for this player
                if wither_turn > 0 and self.half_turn > wither_turn:
                ...
                # Clear Lowlands Mist wither (flag = -1) at end of controller's turn
                if wither_turn == -1:
                    if current_player['battlefield_wither'][i] > 0:
                        self.log(f"{CARDS_BY_ID[current_player['battlefield'][i]]['name']}'s Lowlands Mist wither expires")
                        current_player['battlefield_wither'][i] = 0
                        current_player['battlefield_wither_applied_turn'][i] = 0
    ...
    def start_turn(self):
        ...
        # This Wither expires at END of YOUR turn (not using half_turn)
        ...
        if current_player.get('field') == 'miasma_lowlands_mist':
            for i in range(5):
                if opponent['battlefield'][i] is not None:
                    opponent['battlefield_wither'][i] += 1
                    # Mark as Lowlands wither (expires at end of controller's turn)
                    opponent['battlefield_wither_applied_turn'][i] = -1  # Special flag for Lowlands
                    unit_name = CARDS_BY_ID[opponent['battlefield'][i]]['name']
                    self.log(f"🌫️ Lowlands Mist: {unit_name} withers (DEF -1 until end of turn)")
