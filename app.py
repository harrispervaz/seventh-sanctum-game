...
class GameState:
    ...
    def start_turn(self):
        ...
        # Ready all units (remove exhaustion) - CHECK FOR FL of Slumber first
        opponent_idx = 1 - self.active_player
        for i in range(5):
            if current_player['battlefield'][i] is not None and current_player['battlefield_exhausted'][i]:
                ...
                if flute_traps and opponent['energy'] >= flute_traps[0]['trap']['cost']:
                    # Auto-activate Flute of Slumber
                    trap_info = flute_traps[0]
                    trap_id = trap_info['trap']['id']
                    trap_slot = trap_info['slot']
                    
                    # Pay energy
                    opponent['energy'] -= trap_info['trap']['cost']
                    self.log(f"🎵 Flute of Slumber activated!")
                    
                    # Remove trap
                    opponent['traps'][trap_slot] = None
                    opponent['discard'].append(trap_id)
                    
                    # Resolve trap (keep unit exhausted)
                    self.resolve_trap_effect(trap_id, opponent_idx, {
                        'readied_player': self.active_player,
                        'readied_index': i
                    })
                    
                    # Don't ready this unit
                    self.log(f"{unit_name} remains exhausted (Flute of Slumber)")
                else:
                    # No trap or can't afford - ready the unit normally
...
def activate_trap(game_id):
    ...
    if activate:
        ...
        if counter_sigil_traps and game.players[opponent_idx]['energy'] >= counter_sigil_traps[0]['trap']['cost']:
            # Auto-activate Counter-Sigil
            cs_trap_info = counter_sigil_traps[0]
            cs_trap_id = cs_trap_info['trap']['id']
            cs_trap_slot = cs_trap_info['slot']
            
            # Pay energy for Counter-Sigil
            game.players[opponent_idx]['energy'] -= cs_trap_info['trap']['cost']
            game.log(f"🚫 Counter-Sigil activated!")
            
            # Remove Counter-Sigil
            game.players[opponent_idx]['traps'][cs_trap_slot] = None
            game.players[opponent_idx]['discard'].append(cs_trap_id)
            
            # Resolve Counter-Sigil (negates the original trap)
            cs_result = game.resolve_trap_effect(cs_trap_id, opponent_idx, {
                'enemy_trap_player': player,
                'enemy_trap_name': trap['name']
            })
            
            # Return that trap was negated
            return jsonify({
                'success': True,
                'message': f"{trap['name']} negated by Counter-Sigil!",
                'trap_activated': True,
                'trap_id': trap_id,
                'trap_negated': True,
                'counter_sigil_activated': True,
                'effect_result': cs_result,
                'state': game.get_state(0)
            })
